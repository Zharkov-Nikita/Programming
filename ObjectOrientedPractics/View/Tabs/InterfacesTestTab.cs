...
{
    public partial class InterfacesTestTab : UserControl
    {
        public InterfacesTestTab()
        {
            InitializeComponent();
        ...
        private void RefreshItemsListBoxItem()
        {
            ...
            ItemsListBox2.Items[ItemsListBox.SelectedIndex] = _item;
        }
        ...
        private void ItemCloneButton_Click(object sender, EventArgs e)
        {
            if (ItemsListBox.SelectedIndex != -1)
            {
                Item item = (Item)_currentItem.Clone();
                _items.Add(item);
                ClearItemInfo();
            }
        }

        private void ItemEquialsButton_Click(object sender, EventArgs e)
        {
            try
            {
                MessageBox.Show(_currentItem.Equals(Items[ItemsListBox2.SelectedIndex]).ToString());
            }
            catch { }
        }

        private void ItemCompareButton_Click(object sender, EventArgs e)
        {
            try
            {
                MessageBox.Show(_currentItem.CompareTo(Items[ItemsListBox2.SelectedIndex]).ToString());
            }
            catch { }
        }
    }
